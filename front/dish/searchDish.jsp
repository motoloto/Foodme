...
pageContext.setAttribute("bannerList", bannerList);
	...
	List<DishVO> dishlist = dishSvc.query(dish_cont);
							...
							<div class="carousel-inner">
								<c:forEach var="banner" items="${bannerList}" varStatus="status">
									<div class="item  ${status.index==0?'active':''}">
										<a
											href="<%=getServletContext().getContextPath()%>/front/restaurant/fe_restInfo.jsp?rest_no=${banner.rest_no}">
											<img
					...
					<div class="row clearfix">
						<c:forEach var="dishVO" items="${dishlist}">
							<div class="col-md-4">
								<div class="sm_ad_block">
									<a
										href="<%=getServletContext().getContextPath()%>/front/restaurant/fe_restInfo.jsp?rest_no=${dishVO.rest_no}">
											...
											<c:forEach var="restVO" items="${restSvc.all}">
												<c:if test="${dishVO.rest_no==restVO.rest_no}">
													<tr>
														<td>【${restVO.rest_name}】：
															<h4>
																<b>${dishVO.dish_name }</b>
															</h4>
														</td>
													</tr>
													<tr>
														<td>地址: ${restVO.rest_addr}</td>
													</tr>
													<tr>
														<td>電話: ${restVO.rest_tel}</td>
													</tr>
													<tr>
														<td>營業時間: ${restVO.rest_opentime}</td>
													</tr>
													<tr>
														<td>後位時間: ${restVO.rest_waitmin}</td>
													</tr>
												</c:if>
											</c:forEach>
										</table>
